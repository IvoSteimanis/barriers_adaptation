...
{p 8 40 2}
{it:void}{bind:                         }
{it:u}{cmd:.u_fput(}{it:real scalar fh}{cmd:,}
...
{p 8 40 2}
{it:void}{bind:                         }
{it:u}{cmd:.u_swap_valuelabel(}{it:string rowvector oldnames}{cmd:,}
{it:string rowvector newnames}{cmd:)}
...
{pstd}
{it:u}{cmd:.u_fput()} is a wrapper for {helpb mf_fopen:fput()}; if an 
error occurs, the function closes {it:fh} and exits with a Stata error 
code.
...
{pstd}
{it:u}{cmd:.u_swap_valuelabel()} detaches value labels {it:oldnames} from 
variables and attaches value labels {it:newnames} to variables. {it:oldnames} 
and {it:newnames} must have the same length and correspond one-to-one. Also, 
{it:oldnames} must be unique. Note that the function does not perfrom any 
checks and always produces a Mata traceback log on errors.  
